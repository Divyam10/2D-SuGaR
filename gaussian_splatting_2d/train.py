...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if viewpoint_cam.world_normal_map is not None:
            world_normal_map = torch.from_numpy(viewpoint_cam.world_normal_map).to(args.data_device)
        ...
        normal_prior_loss = 0
        if world_normal_map is not None:
            normal_prior_error = (1 - (rend_normal * world_normal_map).sum(dim=0))[None]
            # normal_prior_loss = (0.1 - (iteration / opt.iterations) * 0.06) * normal_prior_error.mean()
            normal_prior_loss = opt.lambda_normal_prior * normal_prior_error.mean()

        dist_loss = lambda_dist * (rend_dist).mean()

        # loss
        total_loss = loss + dist_loss + normal_loss + normal_prior_loss
