...
class GaussianSplattingWrapper:
    ...
    def get_mask(self, camera_indices:int, to_cuda=False):
        """Returns the mask corresponding to the training camera at the given index.

        Args:
            camera_indices (int): Index of the camera in the set of cameras.
            to_cuda (bool, optional): If True, moves the mask to GPU. Defaults to False.

        Returns:
            Tensor: The mask, or None if no mask is available.
        """
