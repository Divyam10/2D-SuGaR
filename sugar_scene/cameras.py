    ...
    for cam_idx in range(len(camera_transforms)):
        ...
        if load_gt_images:
            ...
            if mask_dir is not None:
                # Try different naming patterns for DTU and other datasets
                mask_paths_to_try = [
                    os.path.join(mask_dir, name + '.png'),  # Same as image name
                    os.path.join(mask_dir, name + '.jpg'),
                    os.path.join(mask_dir, name),  # No extension
                ]

                # For DTU dataset: try removing leading zeros (0000 -> 000, 0001 -> 001, etc.)
                if name.isdigit() and len(name) >= 4:
                    # Try with one less leading zero
                    name_3digit = name.lstrip('0').zfill(3)
                    mask_paths_to_try.extend([
                        os.path.join(mask_dir, name_3digit + '.png'),
                        os.path.join(mask_dir, name_3digit + '.jpg'),
                        os.path.join(mask_dir, name_3digit),
                    ])

                mask_path = None
                for try_path in mask_paths_to_try:
                ...
                if mask_path is not None:
                    ...
                    # Normalize mask to [0, 1] range and ensure it's binary
                    mask = (resized_mask > 0.5).float()
                else:
                    print(f"Warning: Mask not found for {name} (tried multiple naming patterns)")
        ...
        gs_camera = GSCamera(
            colmap_id=id, image=gt_image, gt_alpha_mask=None, mask=mask,
...
class GSCamera(torch.nn.Module):
    ...
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
                 ...
                 image_height=None, image_width=None, mask=None,
                 ):
        """
        Args:
            ...
            mask (_type_, optional): Foreground mask for the image. Defaults to None.
