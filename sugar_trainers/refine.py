...
def refined_training(args):
    ...
    # # Improved learning rates for better mesh quality:
    # position_lr_init=0.0003  # Increased from 0.00016 - better positioning
    # position_lr_final=0.000003  # Increased proportionally
    # feature_lr=0.002  # Decreased from 0.0025 - more stable appearance
    # scaling_lr=0.001  # DECREASED from 0.005 - prevents stretching!
    # rotation_lr=0.002  # Increased from 0.001 - better surface alignment
    ...
    if use_densifier:
        ...
        if initialize_from_trained_3dgs:
            densify_from_iter = 500  # ENABLED: was 500 + 99999 (disabled)
            densify_until_iter = 7000  # ENABLED: was 7000 - 7000 = 0 (disabled)
        else:
    ...
    enforce_background_regularization = True
    ...
    if hasattr(args, 'mask_dir') and args.mask_dir is not None:
        mask_dir = args.mask_dir
        if not os.path.exists(mask_dir):
            CONSOLE.print(f"Warning: Specified mask directory does not exist: {mask_dir}")
            mask_dir = None
        else:
            CONSOLE.print(f"Using specified mask directory: {mask_dir}")
    else:
        mask_dir = os.path.join(source_path, 'mask')
        if not os.path.exists(mask_dir):
            mask_dir = None
            CONSOLE.print("No mask directory found. Training without masks.")
        else:
            CONSOLE.print(f"Found mask directory: {mask_dir}")
    ...
    if loss_function == 'l1':
        ...
        def loss_fn(pred_rgb, gt_rgb, mask=None):
            return (1.0 - dssim_factor) * l1_loss(pred_rgb, gt_rgb, mask) + dssim_factor * (1.0 - ssim(pred_rgb, gt_rgb, mask=mask))
    ...
    for batch in range(9_999_999):
        ...
        for i in range(0, train_num_images, train_num_images_per_batch):
            ...
            if not no_rendering:
                ...
                mask = nerfmodel.get_mask(camera_indices=camera_indices)
                if mask is not None:
                    mask = mask.view(-1, sugar.image_height, sugar.image_width, 1)
                    mask = mask.transpose(-1, -2).transpose(-2, -3)

                # Compute loss
                loss = loss_fn(pred_rgb, gt_rgb, mask)

                if mask is not None and mask_dir is not None and enforce_background_regularization:
                    # Penalize rendering in background regions (where mask is 0)
                    background_mask = 1.0 - mask  # Invert mask to get background regions
                    background_penalty = (pred_rgb * background_mask).abs().mean()

                    background_penalty_weight = 0.3  
                    loss = loss + background_penalty_weight * background_penalty
            ...
            if iteration==1 or iteration % print_loss_every_n_iterations == 0:
                ...
                with torch.no_grad():
                    ...
                    # Save visualization images if masks are used
                    if mask_dir is not None and (iteration == 1 or iteration % (print_loss_every_n_iterations * 5) == 0):
                        vis_dir = os.path.join(sugar_checkpoint_path, 'mask_visualizations')
                        os.makedirs(vis_dir, exist_ok=True)

                        # Save visualization for the current training image
                        import torchvision
                        with torch.no_grad():
                            # Get current mask
                            current_mask = nerfmodel.get_mask(camera_indices=camera_indices)
                            if current_mask is not None:
                                current_mask_viz = current_mask.view(-1, sugar.image_height, sugar.image_width, 1)
                                current_mask_viz = current_mask_viz.transpose(-1, -2).transpose(-2, -3)

                                # Create visualization grid
                                # Convert single channel mask to 3 channels for visualization
                                mask_viz_rgb = current_mask_viz.expand(-1, 3, -1, -1)

                                # Apply mask to ground truth for visualization
                                masked_gt = gt_rgb * current_mask_viz

                                # Create a grid: [GT, Mask, Masked GT, Rendered]
                                vis_grid = torch.cat([gt_rgb, mask_viz_rgb, masked_gt, pred_rgb], dim=0)

                                # Save the grid
                                save_path = os.path.join(vis_dir, f'iter_{iteration:05d}_cam_{camera_indices.item():03d}.png')
                                torchvision.utils.save_image(vis_grid, save_path, nrow=4, normalize=False, value_range=(0, 1))
                                CONSOLE.print(f"Saved mask visualization to {save_path}")
    ...
    if export_ply_at_the_end:
        ...
        tmp_list = os.path.abspath(model_path).split(os.sep)
